...
import com.openclassrooms.micro_bed_availbility.infra.repository.HospitalRepository;
import com.openclassrooms.micro_bed_availbility.infra.repository.SpecialityRepository;
...
public class BedsLoader {
    ...
    public void loadAllBeds(){
        if (bedsRepository.count() > 0) return; // ne pas réinitialiser si déjà peuplé

        List<HospitalEntity> hospitals = hospitalRepository.findAll();

        hospitals.forEach(hospital ->
                hospital.getSpecialities().forEach(speciality -> {
                    int totalBeds     = random.nextInt(10)*10 + 1;  // entre 10 et 100
                    int bookedBeds = random.nextInt(totalBeds + 1); // entre 0 et totalBeds

                    BedsEntity bed = new BedsEntity();
                    bed.setHospital(hospital);
                    bed.setSpeciality(speciality);
                    bed.setTotalBeds(totalBeds);
                    bed.setBookedBeds(bookedBeds);
